...
test.describe('Open Application', () => {
  ...
  test('Open Application - PAP a filter', async ({ page }) => {
    ...
    const filteredItems = await items.filter((item) => item.textContent.includes(''));
  ...
  test('Open Application - detail', async ({ page }) => {
    ...
    // skontroluj, ci klikom na tlacidlo 'Apply now' ta presmeruje na formular


    // skontroluj, ci formular ma url 'https://jobs.datacruit.com/en/reaction/open-application-vissim-no-172275?live=1&share_id=7321C4CB'
